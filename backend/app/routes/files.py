...
async def upload_file(
    ...
    if not workspace or not workspace.get("is_member"):
        if not workspace or workspace.get("is_private"):
            raise HTTPException(status_code=403, detail="Not authorized to upload files to this workspace")
    ...
    file_url = f"/uploads/{workspace_id}/{unique_filename}"
